...
LDFLAGS := -s -w -X github.com/halper/opendj_exporter.commit=${GITCOMMIT}
LDFLAGS := ${LDFLAGS} -X github.com/halper/opendj_exporter.tag=${GIT_TAG}
OUTFILE ?= opendj_exporter
...
ifeq (, $(shell which goimports))
	...
	@goimports -w -local github.com/halper/opendj_exporter $(shell find . -type f -name '*.go' | grep -v '/vendor/')
...
.PHONY: compile
compile: target
	go build -ldflags "${LDFLAGS}" -o target/${OUTFILE} ./cli/...
	gzip -c < target/${OUTFILE} > target/${OUTFILE}.gz

.PHONY: cross-compile
cross-compile:
	OUTFILE=opendj_exporter-linux-amd64 GOOS=linux GOARCH=amd64 $(MAKE) compile
